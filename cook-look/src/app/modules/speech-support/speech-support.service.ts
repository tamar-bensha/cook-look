...
export class SpeechSupportService {
  ...
  constructor(public dialog: MatDialog, private zone: NgZone, private snackBar: MatSnackBar, private route:ActivatedRoute, private router:Router) {
    this.init();
  }
  ...
  private handleEndEvent(event: any): void {
    ...
    if (this._lastResult) {
      switch (this._lastResult.transcript){
      case "new recipe":{this.router.navigateByUrl('/recipe'); break;}
      case "read more":{this.router.navigateByUrl('/about'); break;}
      case "gallery":{this.router.navigateByUrl('/galery'); break;}
      case "homepage":{this.router.navigateByUrl('/'); break;}
